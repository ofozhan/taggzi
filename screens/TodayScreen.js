...
import {
  ...
  FlatList, StyleSheet, Keyboard, TouchableWithoutFeedback, Alert, ScrollView
} from 'react-native';
...
const formatCurrency = (number) => {
  ...
  return new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(roundedUpValue);
};
...
export default function TodayScreen() {
  ...
  useEffect(() => {
    ...
    // GÜNCELLENDİ: 'sound' bağımlılığı eklendi
  }, [sound]);
  ...
  useEffect(() => {
    ...
    saveData();
    // GÜNCELLENDİ: 'isDataLoaded' ve 'today' bağımlılıkları eklendi
